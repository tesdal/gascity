...
// Initializer is the domain contract for city scaffolding +
// finalization. Exactly one implementation exists per process,
// supplied at supervisor construction (see
// internal/api.SupervisorMux.SetInitializer). Both projections —
// CLI and HTTP API — call Init on the same Initializer instance.
